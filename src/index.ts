...
const WORKING_INDICATOR_FRAMES = ["󰪞", "󰪟", "󰪠", "󰪡", "󰪢", "󰪣", "󰪤", "󰪥"] as const;
...
function setupCustomUI(pi: ExtensionAPI, ctx: ExtensionContext) {
...
function getContextLabel(ctx: ExtensionContext): string {
  ...
  const percent = typeof usageInfo.percent === 'number'
    ? usageInfo.percent
    : typeof usageInfo.fraction === 'number'
      ? usageInfo.fraction * 100
      : typeof usageInfo.used === 'number' && typeof usageInfo.limit === 'number' && usageInfo.limit > 0
...
function buildCoreFooterSections(
